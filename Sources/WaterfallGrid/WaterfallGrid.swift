...
extension WaterfallGrid {
    ...
    public init(_ data: Binding<Data>, id: KeyPath<Data.Element, ID>, content: @escaping (Data.Element) -> Content) {
        self._data = data
        self.dataId = id
...
extension WaterfallGrid where ID == Data.Element.ID, Data.Element : Identifiable {
    ...
    /// - Parameter content: A function that can be used to generate content on demand given underlying data.
    public init(_ data: Binding<Data>, content: @escaping (Data.Element) -> Content) {
        self._data = data
